...
function filterBacklinks(backlinks, config = DEFAULT_FILTER_CONFIG, blacklistSet = null, librarySet = null) {
  ...
  const filtered = backlinks.filter(bl => {
    ...
    if (blacklistSet && bl.sourceUrl) {
      if (blacklistSet.has(normalizeUrl(bl.sourceUrl))) {
    ...
    if (librarySet && bl.sourceUrl) {
      if (librarySet.has(normalizeUrl(bl.sourceUrl))) {
  ...
  if (c.deduplicateByDomain) {
    ...
    result = filtered.filter(bl => {
      ...
      if (seenDomains.has(bl.sourceDomain)) {
