...
// Like addRecords, but swallow ConstraintError (unique-index conflicts) so the
// transaction is not aborted by duplicates. Real failures still reject via
// tx.onabort. Use only for stores where duplicates are expected and benign
// (e.g. DISCOVERED_SITES), not for authoritative writes.
async function addRecordsBestEffort(storeName, records) {
  const db = await openDB();
  return new Promise((resolve, reject) => {
    const tx = db.transaction(storeName, 'readwrite');
    const store = tx.objectStore(storeName);
